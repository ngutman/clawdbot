...
describe("context-pruning", () => {
  ...
  it("reads per-session settings from env", async () => {
    const prev = process.env.CLAWDBOT_PI_CONTEXT_PRUNING;
    try {
      process.env.CLAWDBOT_PI_CONTEXT_PRUNING = JSON.stringify({
        v: 1,
        bySessionId: {
          "sess-a": {
            config: {
      ...
      const api = {
        ...
        appendEntry: (_type: string, _data?: ContextPruningLogEntry) => {},
      ...
      if (typeof prev === "string") {
        process.env.CLAWDBOT_PI_CONTEXT_PRUNING = prev;
      } else {
        delete process.env.CLAWDBOT_PI_CONTEXT_PRUNING;
      }
    }
  });
