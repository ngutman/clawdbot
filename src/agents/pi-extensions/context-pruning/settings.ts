...
export type ContextPruningLogMode = "changes" | "always";

export type ContextPruningLogConfig = {
  enabled?: boolean;
  mode?: ContextPruningLogMode;
};
...
export function computeEffectiveSettings(
  ...
  if (cfg.log) {
    if (typeof cfg.log.enabled === "boolean") {
      s.log.enabled = cfg.log.enabled;
    }
    if (cfg.log.mode === "changes" || cfg.log.mode === "always") {
      s.log.mode = cfg.log.mode;
    }
  }
  if (cfg.softTrim) {
    if (
